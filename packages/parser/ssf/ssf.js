...
const write_num = (function make_write_num() {
  ...
  function write_num_pct(type, fmt, val) {
    ...
    return write_num(type, sfmt, val * 10 ** (2 * mul)) + fill('%', mul);
  }
  ...
  function write_num_flt(type, fmt, val) {
    ...
    if ((r = fmt.match(dec1))) {
      o = rnd(val, r[1].length)
        ...
        .replace(
          /\.(\d*)$/,
          ($$, $1) => `.${$1}${fill('0', hashq(r[1]).length - $1.length)}`
        );
    ...
    if ((r = fmt.match(/^#,##0\.([#0]*0)$/))) {
      return val < 0
        ...
        : `${commaify(`${Math.floor(val) + carry(val, r[1].length)}`)}.${pad0(
            dec(val, r[1].length),
            r[1].length
          )}`;
    }
  ...
  while (i < fmt.length) {
    switch ((c = fmt.charAt(i))) {
      ...
      case '#':
        o = c;
        while (
          (++i < fmt.length && '0#?.,E+-%'.indexOf((c = fmt.charAt(i))) > -1) ||
          (c == '\\' &&
            fmt.charAt(i + 1) == '-' &&
            i < fmt.length - 2 &&
            '0#'.indexOf(fmt.charAt(i + 2)) > -1)
      ...
      case ')':
        out[out.length] = {
          t: flen === 1 || (typeof v === 'number' && v < 0) ? 't' : c,
          v: c,
        };
        ++i;
        break;
