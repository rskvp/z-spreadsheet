...
const countIf = (range, criteria) => {
  ...
  const arr = new Array(range.length * range[0].length).fill(false);
  // parse criteria
    ...
    row.forEach((value) => {
      ...
      if (criteria.op === 'wc') {
        ...
        if (typeof x === 'boolean') {
          ...
          x.forEach((row) => row.forEach((val) => (localBool |= val)));
          bool = localBool;
        }
...
const StatisticalFunctions = {
  ...
  COUNT: (...ranges) => {
    ...
    H.flattenParams(ranges, null, true, (item, info) => {
      ...
      if (info.isLiteral && !isNaN(item)) {
        cnt++;
      } else if (typeof item === 'number') cnt++;
    });
    return cnt;
  },
  ...
  COUNTIFS: (range1, criteria1, ...criteriaList) => {
    ...
    len = arrs[0].length;
    const arr = new Array(len).fill(true);
  ...
  MINIFS: (minRange, ...otherCriteria) => {
    if (otherCriteria.length > 252) {
      throw FormulaError.TOO_MANY_ARGS;
    }

    const range = H.flattenDeep(H.accept(minRange));
    const criteria_ranges = [];
    const criteria = [];

    otherCriteria.forEach((val, idx) => {
      const accepted = H.accept(val);
      if (idx % 2 === 0 && typeof accepted !== 'object') {
        throw FormulaError.VALUE;
      }
      if (idx % 2 === 0) {
        return criteria_ranges.push(H.flattenDeep(accepted));
      }
      criteria.push(accepted);
    });

    if (criteria.length != criteria_ranges.length) {
      throw FormulaError.VALUE;
    }
    criteria_ranges.forEach((val) => {
      if (val.length != range.length) {
        throw FormulaError.VALUE;
      }
    });
    ...
    range.forEach((val, idx) => {
      const allTrue = criteria_ranges.every(
        (prev, curr, criteriaIdx) => prev && curr[idx] == criteria[criteriaIdx]
      );
      if (allTrue) {
        arr.push(H.accept(val, Types.NUMBER));
      }
    });
    return Math.min(...arr);
  },
