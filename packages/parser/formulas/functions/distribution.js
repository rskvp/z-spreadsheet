...
const DistributionFunctions = {
  ...
  'CONFIDENCE.T': (alpha, std, size) => {
    ...
    if (size === 1) throw FormulaError.DIV0;
  ...
  CORREL: (array1, array2) => {
    ...
    if (array1.length !== array2.length) throw FormulaError.NA;
    ...
    if (filterArr1.length <= 1) throw FormulaError.DIV0;
  ...
  'COVARIANCE.P': (array1, array2) => {
    ...
    const mean1 = jStat.mean(filterArr1);
    const mean2 = jStat.mean(filterArr2);
    let result = 0;
  ...
  'EXPON.DIST': (x, lambda, cumulative) => {
    ...
    cumulative = H.accept(cumulative, Types.BOOLEAN);
    if (x < 0 || lambda <= 0) throw FormulaError.NUM;
    return cumulative
  ...
  FORECAST: (x, knownYs, knownXs) => {
    ...
    if (knownXs.length !== knownYs.length) throw FormulaError.NA;
    ...
    for (let i = 0; i < knownYs.length; i++) {
      ...
      if (knownXs[i] !== knownXs[0]) xAllEqual = false;
    }
    if (xAllEqual) throw FormulaError.DIV0;
  ...
  'FORECAST.LINEAR': (...params) => DistributionFunctions.FORECAST(...params),
  ...
  GROWTH: (knownY, knownX, newX, useConst) => {
    ...
    for (let i = 0; i < knownY.length; i++) {
      if (typeof knownY[i] !== 'number') throw FormulaError.VALUE;
    }
    ...
    if (knownX == null) {
      ...
      if (knownX.length !== knownY.length) throw FormulaError.REF;
      for (let i = 0; i < knownX.length; i++) {
        if (typeof knownX[i] !== 'number') throw FormulaError.VALUE;
      }
    }
    ...
    for (let i = 0; i < newX.length; i++) {
      ...
      for (let j = 0; j < newX[0].length; j++) {
        if (typeof newX[i][j] !== 'number') throw FormulaError.VALUE;
  ...
  INTERCEPT: (knownYs, knownXs) => {
    ...
    if (filteredY.length <= 1) throw FormulaError.DIV0;
    const yMean = jStat.mean(filteredY);
    const xMean = jStat.mean(filteredX);
    let numerator = 0;
    let denominator = 0;
  ...
  'T.DIST': (x, deg_freedom, cumulative) => {
    ...
    return cumulative
      ? jStat.studentt.cdf(x, deg_freedom)
      : jStat.studentt.pdf(x, deg_freedom);
  },
