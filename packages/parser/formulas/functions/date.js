...
function stringIsDate(date) {
  const dateFormats = [
    'YYYY-MM-DD HH:mm:ss', // UTC date format
    'DD-MMM-YY',
    'MM/D/YY HH:mm A',
    'D-MMM',
    'MMM-D HH:mm',
...
const DateFunctions = {
  DATE: (year, month, day) => {
    ...
    if (year < 0 || year >= 10000) throw FormulaError.NUM;
  ...
  DATEDIF: (startDate, endDate, unit) => {
    ...
    if (startDate > endDate) throw FormulaError.NUM;
  ...
  DATEVALUE: (dateText) => {
    ...
    if (serial < 0 || serial > 2958465) throw FormulaError.VALUE;
    return serial;
  },
  ...
  DAYS: (endDate, startDate) => {
    ...
    startDate = parseDate(startDate);
    let offset = 0;
    if (startDate < -2203891200000 && endDate > -2203891200000) {
      offset = 1;
    }
  ...
  ISDATE: (dates) => {
    const allDates = H.accept(dates);
    if (typeof allDates === 'string') {
      return stringIsDate(allDates);
    }
    allDates.map((row) => {
      row.map((value) => stringIsDate(value));
    });
    throw FormulaError.CUSTOM(
      'Multiple testing for multiple ranges is not yet supported'
    );
    return allDates;
  },
  ...
  WEEKDAY: (serialOrString, returnType) => {
    ...
    if (!weekTypes) throw FormulaError.NUM;
  ...
  WORKDAY: (startDate, days, holidays) =>
    DateFunctions['WORKDAY.INTL'](startDate, days, 1, holidays),
  ...
  TO_DATE: (n) => JSON.stringify({ result: n.value, resultType: 'date' }),
};
