...
const MathFunctions = {
  ...
  EVEN: (number) => MathFunctions.CEILING(number, -2),
  ...
  'ISO.CEILING': (...params) => MathFunctions['CEILING.PRECISE'](...params),
  ...
  PRODUCT: (...numbers) => {
    ...
    H.flattenParams(
      ...
      (number, info) => {
        ...
        } else if (typeof number === 'number') product *= number;
      },
      1
    );
    return product;
  ...
  SUM: (...params) => {
    ...
    H.flattenParams(params, Types.NUMBER, true, (item, info) => {
      ...
      if (info.isLiteral) {
        result += item;
      } else if (typeof item === 'number') result += item;
    });
    return result;
  },
  ...
  SUMIFS: (context, inputRange, ...criteriaList) => {
    ...
    sums = sums.filter((val) => {
       ...
       */
      return !toRemove.find(
        ({ row, col }) => row === val.row && col === val.col
      );
    });
    ...
    arrays.forEach((array) => {
      ...
      for (let i = 0; i < array1.length; i++) {
        for (let j = 0; j < array1[0].length; j++) {
          if (typeof array1[i][j] === 'boolean' && array1[i][j]) {
            array1[i][j] = 1;
          } else if (typeof array1[i][j] !== 'number') array1[i][j] = 0;
          if (typeof array[i][j] === 'boolean' && array[i][j]) array[i][j] = 1;
          else if (typeof array[i][j] !== 'number') array[i][j] = 0;
  ...
  SUMSQ: (...params) => {
    ...
    H.flattenParams(params, Types.NUMBER, true, (item, info) => {
      ...
      if (info.isLiteral) {
        result += item ** 2;
      } else if (typeof item === 'number') result += item ** 2;
    });
    return result;
  },
