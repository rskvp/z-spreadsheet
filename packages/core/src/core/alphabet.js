...
export const REGEX_EXPR_NONGLOBAL_AT_START =
  /^[$]?[a-zA-Z]{1,3}[$]?[1-9][0-9]*/;
export const REGEX_EXPR_RANGE_GLOBAL =
  /[$]?[a-zA-Z]{1,3}[$]?[1-9][0-9]*:[$]?[a-zA-Z]{1,3}[$]?[1-9][0-9]*/g;
export const REGEX_EXPR_RANGE_NONGLOBAL_AT_START =
...
export function expr2cellRangeArgs(src) {
  ...
  if (
    srcIndexEndOfStartRef >= src.length ||
    src[srcIndexEndOfStartRef] != ':'
  ) {
    return [sri, sci, sri, sci];
  }

  const endRef = expr2xy(src.slice(srcIndexEndOfStartRef + 1));
...
export function xy2expr(x, y, xIsAbsolute = false, yIsAbsolute = false) {
  const insertAbs = function (isAbsolute) {
    return isAbsolute ? '$' : '';
  };
  return `${insertAbs(xIsAbsolute)}${stringAt(x)}${insertAbs(yIsAbsolute)}${
    y + 1
  }`;
}
