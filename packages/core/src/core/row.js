              ...
              for (let jj = dsci; jj <= deci; jj += cn) {
                ...
                if (
                  ...
                  if (ncellState.text[0] === '=') {
                    ncellState.text = ncellState.text.replace(
                      REGEX_EXPR_GLOBAL,
                      (word) => {
                        let [xn, yn] = [0, 0];
                        if (sri === dsri) {
                    ...
                    if (result !== null) {
                      ...
                      ncellState.text =
                        ncellState.text.substring(0, result.index) + index;
                    }
                  }
                }
    ...
    this.each((ri, row) => {
      this.eachCells(ri, (ci, cell) => {
        ...
        if (isFormula(cellText)) {
          cell.setText(
            cellText.replace(REGEX_EXPR_GLOBAL, (word) =>
  ...
  delete(sri, eri) {
    ...
    this.each((ri, row) => {
      ...
      if (nri < sri) {
        ...
        // they depend on.
        this.eachCells(ri, (ci, cell) => {
          cell.uses.forEach((dependentCell) =>
            dependentCell.noLongerUsedByCell(cell)
          );
        });
      }
    });
    ...
    this.each((ri, row) => {
      this.eachCells(ri, (ci, cell) => {
        ...
        if (isFormula(cellText)) {
          // Adjust ranges
          let newCellText = cellText.replace(
            REGEX_EXPR_RANGE_GLOBAL,
            (word) => {
              ...
              const [
                rangeStartX,
                rangeStartY,
                rangeStartXIsAbsolute,
                rangeStartYIsAbsolute,
              ] = expr2xy(rangeStart);
              const [
                rangeEndX,
                rangeEndY,
              ...
              const isRangeStartInDeletedZone =
                rangeStartY >= sri && rangeStartY <= eri;
              const isRangeEndInDeletedZone =
                rangeEndY >= sri && rangeEndY <= eri;
        ...
        if (isFormula(cellText)) {
          ...
          let newCellText = cellText.replace(
            ...
            (word) => {
              ...
              const isRangeStartInDeletedZone =
                rangeStartX >= sci && rangeStartX <= eci;
              const isRangeEndInDeletedZone =
                rangeEndX >= sci && rangeEndX <= eci;
