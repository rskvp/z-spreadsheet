...
function setStyleBorder(ri, ci, bss) {
  ...
  let cstyle = {};
  if (cell.state.style !== undefined) {
    cstyle = helper.cloneDeep(styles[cell.state.style]);
  }
  cstyle = helper.merge(cstyle, { border: bss });
  cell.state.style = this.addStyle(cstyle);
}
...
function setStyleBorders({ mode, style, color }) {
  ...
  if (mode === 'outside' && !multiple) {
    ...
    selector.range.each((ri, ci) => {
      ...
      if (cell && cell.state.style !== undefined) {
        const ns = helper.cloneDeep(styles[cell.state.style]);
        delete ns.border;
        // ['bottom', 'top', 'left', 'right'].forEach((prop) => {
        //   if (ns[prop]) delete ns[prop];
        // });
        cell.state.style = this.addStyle(ns);
      }
    });
  } else if (
    mode === 'all' ||
...
export default class DataProxy {
  ...
  copyToSystemClipboard() {
    ...
    for (
      ...
      if (rowData.hasOwnProperty(ri)) {
        for (
          var ci = this.selector.range.sci;
          ...
          if (rowData[ri].cells.hasOwnProperty(ci)) {
            const cellText = String(rowData[ri].cells[ci].getText());
            if (
              cellText.indexOf('\n') == -1 &&
              cellText.indexOf('\t') == -1 &&
              cellText.indexOf('"') == -1
  ...
  getCellStyle(ri, ci) {
    const cell = this.getCell(ri, ci);
    if (cell && cell.state.style !== undefined) {
      return this.styles[cell.state.style];
    }
    return null;
  }
  ...
  getCellStyleOrDefault(ri, ci) {
    ...
    const cell = rows.getCell(ri, ci);
    const cellStyle =
      cell && cell.state.style !== undefined ? styles[cell.state.style] : {};
      ...
      if (
        ...
        Object.entries(d.rows).forEach(([rowsProperty, rowsPropertyValue]) => {
          if (rowsProperty !== 'len') {
            ...
            Object.entries(rowsPropertyValue.cells).forEach(
              ([ci, cellData]) => {
