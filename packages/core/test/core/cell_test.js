...
describe('cell', () => {
  describe('.render()', () => {
    it('should return 2 when the value is IF(AND(1=1, 2>1), 2, 1)', () => {
      assert.equal(
        cell.render('=IF(AND(1=1, 2>1), 2, 1)', table.formulaParser),
        2
      );
    });
    it('should return 57 when the value is =(9+(3-1))*(2+3)+4/2', () => {
      assert.equal(
        cell.render('=(9+(3-1))*(2+3)+4/2', table.formulaParser),
        57
      );
    });
    it('should return 0 + 2 + 2 + 6 + 49 + 20 when the value is =SUM(A1,B2, C1, C5) + 50 + B20', () => {
      ...
      assert.equal(
        cell.render('=SUM(A1,B2, C1, C5) + 50 + B20', table.formulaParser),
        0 + 2 + 2 + 6 + 50 + 20
      );
    });
    ...
    it('should return 1 + 500 - 20 when the value is =AVERAGE(A1:A3) + 50 * 10 - B20', () => {
      ...
      assert.equal(
        cell.render('=AVERAGE(A1:A3) + 50 * 10 - B20', table.formulaParser),
        1 + 500 - 20
      );
    });
  });
